...
data$PrixTotal = data$Superficie * data$PrixUnitaire
data$TotalVersement = data$Vers1 + data$Vers2 + data$Vers3
data = select(data,-"TotalAvances")

data$ResteàPayer = data$PrixTotal - data$TotalVersement
nrow(data[data$ResteàPayer < 0,])
data<-data[!(data$ResteàPayer < 0),]

# Every Product should have a Unite Price
data<-data[!(data$PrixUnitaire==0),]
# to purchase a product you need pay the Vers1
data = subset(data, Vers1 != 0)
```
...
sum(data$TotalVersement)
sum(data$TotalAvances)
```
