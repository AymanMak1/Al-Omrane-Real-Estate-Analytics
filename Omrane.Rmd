...
data <- read.csv(file = "./Datasets/Omrane.csv", header = TRUE, sep="\t", dec=",")
#data$X <- NULL
...
data<-data[!(data$PrixUnitaire==0),]



data.without.outliers = subset(data, Vers1 != 0)
...
Removing Outliers
```{r}
boxplot(data$Superficie)
boxplot(data$PrixUnitaire)
data<-data[!(data$PrixUnitaire==2600000.00),]
data<-data[!(data$PrixUnitaire==170300.00),]
data<-data[!(data$Superficie==21020.0),]
```
...
```{r}
densityVers2 = density(data$TotalVersement, bw = 1000)
...
making the data machine learning ready to create create a regression predictive model of Al Omrane Products Prices
...
data.ml = select(data, c(-"Opération",-"Division",-"Client",-"NombreUnité",-"TypeDocument",
                         ...
                         -"TotalAvances",-"Dossier",-"NuméroClient",-"Vers1",-"Vers2",-"Vers3",-"TotalVersement",-"DésignationTypeDeDocument",-"PrixTotal"))
#data.ml = select(data.ml,c(-"DésignationTypeDeDocument"))
summary(data.ml)
colnames(data.ml)[3] <- "TypeUnite"
```

Removing Outliers from the dataset that will be used in creating the machine learning models
```{r}
# Removing Outliers
